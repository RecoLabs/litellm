...
describe("custom tier sets", () => {
  ...
  describe("hydrateCustomTierSet", () => {
    it("round-trips what serializeCustomTierSet wrote, in order, ids synthesized by position", () => {
      const serialized = serializeCustomTierSet(customTierSet);
      const hydrated = hydrateCustomTierSet(serialized);
      expect(hydrated).toEqual({
        tiers: customTierSet.tiers.map((row, index) => ({ ...row, id: `stored-${index}` })),
        fallback_tier_id: "stored-1",
      });
    ...
    it("round-trips a backend-valid shape this UI never writes, preserving severity order", () => {
      ...
      expect(hydrated?.tiers).toEqual([
        { id: "stored-0", name: "SIMPLE", definition: "only greetings", models: ["cheap"] },
        { id: "stored-1", name: "COMPLEX", definition: "", models: ["big"] },
      ]);
